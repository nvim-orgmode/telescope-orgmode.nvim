...
---@param name? string Override adapter for this call
---@return table adapter module
local function get_adapter(name)
  local requested = name or adapter_name

  -- Return cached adapter only if it matches the requested one
  if active_adapter and requested == adapter_name and not name then
...
function M.refile_heading(opts)
  return get_adapter(opts and opts.adapter).refile_heading(opts)
end

function M.search_headings(opts)
  return get_adapter(opts and opts.adapter).search_headings(opts)
end

function M.insert_link(opts)
  return get_adapter(opts and opts.adapter).insert_link(opts)
end

function M.search_tags(opts)
  return get_adapter(opts and opts.adapter).search_tags(opts)
end
